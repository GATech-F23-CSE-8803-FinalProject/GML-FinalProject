...
class TFRecordDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for raw_record in raw_dataset:
                cmap, seq, label = parse_tfrecord(raw_record, self.n_goterms, self.channels, self.cmap_type, self.cmap_thresh, self.ont)
                data.append((cmap, seq, label))

        rand_idx = random.randint(0, len(data))
        cmap, seq, label = data[rand_idx]
    ...
    def train_model(self, device, train_tfrecord_fn, valid_tfrecord_fn, epochs=100, batch_size=64, pad_len=1200, cmap_type='ca', cmap_thresh=10.0, ont='mf'):
        ...
        for epoch in range(epochs):
            ...
            for input_cmap, input_seq, labels in train_loader:
                input_cmap = input_cmap.to(device)
